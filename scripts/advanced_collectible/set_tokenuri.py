...
def main():
    ...
    print(f"{number_of_collectibles} tokenIds are here!!")
...
def set_tokenURI(token_id, nft_contract, tokenURI):
    account = get_account()
    tx = nft_contract.tokenURI(token_id, tokenURI, {"from": account})
    tx.wait(1)
    print(f"Awesome! you can view your NFT at {OPENSEA_URL.format(nft_contract.address, token_id)}")
    print("Please wait some minutes and hit the refresh button")
